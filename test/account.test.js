...
describe('app', () => {
	describe('account', () => {
		describe('create', () => {
			it('should create an account', () => request(app)
					.put('/account')
					.send({ owner: { name: 'leo' } })
					.expect(200)
					.then((res) => {
		...
		describe('find', () => {
			it('should retrieve an account', () => request(app)
					.get('/account/1')
					.expect(200)
					.then((res) => {
						should.exist(res.body.account);
		...
		describe('deactivate', () => {
			it('should deactivate an account', () => createAccount()
					.then(account =>
						request(app)
		...
		describe('deposit', () => {
			it('should deposit money into account', () => createAccount()
					.then(account =>
						deposit(account.id, 1000)
							.then(() => getBalance(account.id).should.eventually.equal(1000))
					));

			it('should deposit more money into account', () => createAccount()
					.then(account =>
						deposit(account.id, 1000)
			...
			it('should forfeit negative deposit', () => createAccount()
					.then(account =>
						request(app)
			...
			it('should forfeit zero deposit', () => createAccount()
					.then(account =>
						request(app)
							.post(`/account/deposit/${account.id}`)
							.send({ amount: 0 })
		...
		describe('withdraw', () => {
			it('should forfeit negative withdrawal', () => createAccount()
					.then(account =>
						request(app)
			...
			it('should forfeit zero withdrawal', () => createAccount()
					.then(account =>
						request(app)
							.post(`/account/withdraw/${account.id}`)
							.send({ amount: 0 })
			...
			it('should withdraw money from account', () => createAccount()
					.then(account =>
						deposit(account.id, 1000)
			...
			it('should fail to withdraw excess money from account', () => createAccount()
					.then(account =>
						deposit(account.id, 1000)
							.then(() =>
								request(app)
		...
		describe('transfer', () => {
			...
			it('should transfer money to another account of same owner without charges', () => createAccount(null, randomName())
					.then(accountA => deposit(accountA.id, 1000000)
			...
			it('should fail to transfer without sufficient deposit', () => createAccount(null, randomName())
					.then(accountA => deposit(accountA.id, 5000)
							.then(() => createAccount(accountA.ownerId))
							.then(accountB =>
			...
			it('should fail one of two transfers due to insufficient deposit', () => {
				...
				return createAccount(null, randomName())
					.then(accountA => deposit(accountA.id, 8000)
							.then(() =>
								Promise.all([
			...
			it('should fail due to amount exceeds daily transfer limit', () => createAccount(null, randomName())
					.then(accountA => deposit(accountA.id, 20000)
							.then(() => createAccount(accountA.ownerId))
							.then(accountB => transfer(accountA.id, accountB.id, 11000, 400))));

			it('should fail due to exceeding daily transfer limit', () => createAccount(null, randomName())
					.then(accountA => deposit(accountA.id, 20000)
							.then(() => createAccount(accountA.ownerId))
							.then(accountB =>
			...
			it('should only count today transfer in daily transfer limit', () => createAccount(null, randomName())
					.then(accountA => deposit(accountA.id, 20000)
							.then(() => createAccount(accountA.ownerId))
							.then(accountB =>
								transfer(accountA.id, accountB.id, 6000)
			...
			it('should charge 100 for cross owner transfer', () => Promise.all([
			...
			it('should fail due to insufficient deposit for service charge', () => Promise.all([
				createAccount(null, randomName()),
				createAccount(null, randomName())
			])
				.then(([accountA, accountB]) =>
			...
			it('should fail due to external API fail', () => {
				nock.cleanAll();
				nock(approvalBaseUrl)
					.get(approvalPath)
