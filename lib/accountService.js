...
accountService.create = ownerId => db.query('INSERT INTO accounts (owner_id) VALUES ($1) RETURNING id, owner_id, balance', [ownerId])
		.then((result) => {
...
accountService.find = id => db.query('SELECT id, owner_id, balance FROM accounts WHERE id=$1 and active=true', [id])
		.then((result) => {
...
accountService.deactivate = id => db.query('UPDATE accounts SET active=false WHERE id=$1', [id])
		.then(() => Promise.resolve());

accountService.deposit = (id, amount) => db.query('UPDATE accounts SET balance = balance + $2 WHERE id=$1 and active=true', [id, amount])
		.then(result => result.rowCount > 0);

accountService.withdraw = (id, amount) => db.query('UPDATE accounts SET balance = balance - $2 WHERE id=$1 and active=true and balance >= $2', [id, amount])
		.then(result => result.rowCount > 0);
...
const approvalUrl = 'http://handy.travel/test/success.json';
...
accountService.transfer = (fromAccountId, toAccountId, amount) => {
	...
	return bluebird.coroutine(function* () {
		...
		try {
			...
			yield client.query('UPDATE accounts SET balance = balance + $2 WHERE id=$1 and active=true', [toAccountId, amount]);

			yield client.query('INSERT INTO records (account_id, to_account_id, amount, charge) VALUES ($1, $2, $3, $4) RETURNING account_id, to_account_id, amount', [fromAccountId, toAccountId, amount, charge]);
			...
			if (err instanceof UserError || err instanceof ApprovalError)
